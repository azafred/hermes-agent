...
| `/project [list\|status\|use <project>\|clear\|start]` | Bind one messaging session/thread to a first-class, profile-local Hermes Project. `use` selects by id, slug, or name; `clear` restores the gateway default workspace. On Discord, the native **start** action creates a new thread, binds its project before the first agent turn, and can dispatch an optional first request. Project changes are rejected while that same session is running. |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/project`, and `/commands` are **messaging-only** commands.
